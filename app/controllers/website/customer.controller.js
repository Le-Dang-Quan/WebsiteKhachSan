...
class customerController {
    ...
    async register(req, res) {
        const { FullName, Phone, Email, Username, Password, isAdmin } = req.body;
        ...
        try {
            ...
            if(await Customer.findOne({where: {Email}})) return res.render('website/customer/register', {title: "Hosteller - Đăng ký", error: "Email đã tồn tại trong hệ thống!"})
            
            if(await Customer.findOne({where: {Phone}})) return res.render('website/customer/register', {title: "Hosteller - Đăng ký", error: "Số điện thoại đã tồn tại trong hệ thống!"})
